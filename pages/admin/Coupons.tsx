...
export function AdminCoupons() {
  ...
  const startNew = () => { setEditing(null); setForm({ code: '', type: 'percentage', value: 10, isActive: true }); };
  ...
  const remove = (id: string) => { if (window.confirm('Delete coupon?')) { CouponsAPI.delete(id); setList(CouponsAPI.list()); } };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={startNew}><Plus className="h-4 w-4 mr-2"/>New Coupon</Button>
      </div>
                ...
                <tr key={c.id} className="hover:bg-muted/50">
                  ...
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="sm" className="text-destructive" onClick={() => remove(c.id)}><Trash2 className="h-4 w-4"/></Button>
                    </div>
                  </td>
                </tr>
              ))}
